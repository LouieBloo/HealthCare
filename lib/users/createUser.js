...
var createUserPost = function(req,res,next)
{
	...
	.then(function(){//insert the user into the db
		return new Promise(function(resolve,reject){
			
			var FName = userInformation.FName.value;
			FName = FName.charAt(0).toUpperCase() + FName.slice(1);//capitalize first letter
			var LName = userInformation.LName.value;
			LName = LName.charAt(0).toUpperCase() + LName.slice(1);

			var altPhone = userInformation.AltPhone.value ? userInformation.AltPhone.value : null;
			var language = userInformation.Language.value ? userInformation.Language.value : null;

			switch(userInformation.UserType.value){
				case "2"://consumer
					role = 8;
					subRole = 2;
					break;
			...
			var parameters = [FName,LName,userInformation.Email.value,phone(userInformation.Phone.value)[0],phone(altPhone)[0],addressID,altAddressID,userInformation.DOB.value,language,role,subRole];
	...
	.then(function(){//do extra stuff based on what type of user they are
		return new Promise(function(resolve,reject){

			if(subRole == "2")
			{
				var diag = userInformation.Diagnosis.value ? userInformation.Diagnosis.value : null;
			
				var parameters = [userID,userInformation.UCI.value,diag,1,userInformation.ConsumerType.value];
				database.db.query('INSERT INTO ConsumerDetails (ConsumerID,UCI,Diagnosis,Status,Type) VALUES (?,?,?,?,?)',parameters,function(err,results){
					if(err){
						reject(err);
					}
					else
					{
						res.redirect("/users/consumer/" + userID);
						resolve();
					}
				});
...
var buildDefaultPrefill = function(req,callback)
{
	...
	var ConsumerType = req.body.ConsumerType != null ? req.body.ConsumerType : 1;
	response.ConsumerType = {value: ConsumerType,error: ''};
